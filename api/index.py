...
from .plugins import AGENT_CONFIGS, WebAgentType, get_web_agent
from .schemas import ChatRequest, SessionRequest

# Import our browser_use CDP service
...
# Set the CDP WebSocket endpoint from environment variable
...
async def create_session(request: SessionRequest):
    ...
    """
    # try:
    # Create a new session using our browser_use CDP service
    session = await browser_use_service.create_session(
...
async def release_session(session_id: str):
    ...
    """
    try:
        # Release the session using our browser_use CDP service
        result = await browser_use_service.release_session(session_id)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def handle_chat(request: ChatRequest):
    ...
    try:
        ...
        # The debugUrl will be set by the agent in the response stream
        return response
    except Exception as e:
